...
const LoanCalculatorSection = ({
  ...
  const getMinAmount = () =>
    mode === "loan" ? minLoanAmount : minInvestmentAmount;
  const getMaxAmount = () =>
    mode === "loan" ? maxLoanAmount : maxInvestmentAmount;
            ...
            <div className="bg-white border border-gray-200 shadow-lg rounded-tr-none rounded-tl-lg rounded-bl-lg rounded-br-3xl p-6">
              ...
              <div className="mb-6">
                ...
                {mode === "loan" && !amountError && loanAmount && (
                  <div className="mt-2 text-sm text-gray-500">
                    Entered: ₦{parseAmount(loanAmount).toLocaleString()}{" "}
                    {getRangeDisplay()}
                  </div>
                )}
              </div>
              ...
              <div className="mb-8 relative" ref={dropdownRef}>
                ...
                <div
                  ...
                  onMouseEnter={() =>
                    !isDropdownOpen && setHoveredOption("trigger")
                  }
                  onMouseLeave={() => !isDropdownOpen && setHoveredOption(null)}
                >
                  <span
                    className={loanTenure ? "text-gray-700" : "text-gray-400"}
                  >
                    {loanTenure
                      ? tenureOptions.find((opt) => opt.value === loanTenure)
                          ?.label
                      : tenurePlaceholder}
                  </span>
                  ...
                  <img
                    ...
                    alt="Dropdown arrow"
                    className={`w-5 h-5 text-gray-400 mr-5 transition-transform duration-200 ${
                      isDropdownOpen ? "transform rotate-180" : ""
                    } ${hoveredOption === "trigger" ? "text-purple-600" : ""}`}
                    onError={(e) => {
                      e.currentTarget.onerror = null;
                      e.currentTarget.src =
              ...
              {/* Button - Lighter Purple Shade with mobile text centering */}
              <button
                className="w-full flex items-center justify-center sm:justify-start bg-purple-200 text-purple-800 py-3 rounded-full hover:bg-purple-200 transition-colors duration-200 font-semibold text-base mb-8 disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              >
                <img
                  ...
                  alt="Calculate icon"
                  className="w-10 h-10 mr-25 ml-2 object-contain hidden sm:block"
                  onError={(e) => {
                    e.currentTarget.onerror = null;
                    e.currentTarget.src =
